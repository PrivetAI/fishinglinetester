...
struct TestStatistics: Codable {
    ...
    var failedTests: Int
    var largestCatchWeightLb: Double
    var largestCatchFishName: String
    var closestCallPercentage: Double
    var highestSafetyMargin: Double
    var mostUsedLineType: LineType?
    var favoriteTests: [LineType: Int]
    ...
    init() {
        ...
        successfulTests = 0
        failedTests = 0
        largestCatchWeightLb = 0
        largestCatchFishName = ""
        closestCallPercentage = 0
        highestSafetyMargin = 0
        mostUsedLineType = nil
        favoriteTests = [:]
    }
    ...
    mutating func update(with result: SimulationResult) {
        ...
        if result.success {
            ...
            let margin = result.safetyMargin
            if margin > highestSafetyMargin {
                highestSafetyMargin = margin
            }
        } else {
            failedTests += 1
        }
        ...
        let lineType = result.line.type
        favoriteTests[lineType, default: 0] += 1
        
        // Update most used line type
        mostUsedLineType = favoriteTests.max(by: { $0.value < $1.value })?.key
    }
}
